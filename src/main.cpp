...
const int64_t nTargetSpacing = 60; // Reddcoin: 1 minute block
static const int64_t nTargetTimespan = 60 * 60; // Reddcoin: Not used

//
// maximum nBits value could possible be required nTime after
//
unsigned int static ComputeMaxBits(const CBigNum& bnLimit, unsigned int nBase, int64_t nTime)
{
    ...
    else if (block.IsProofOfStake())
    {
        ...
        if (!::GetCoinAge(block.vtx[1], nCoinAge))
            return state.DoS(100, error("ConnectBlock() : %s unable to get coin age for coinstake",
                             ...
                             REJECT_INVALID, "unable to get coin age");

        int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees);
        if (nStakeReward > nCalculatedStakeReward)
            return state.DoS(100, error("ConnectBlock() : coinstake pays too much(actual=%"PRId64" vs calculated=%"PRId64")",
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        if (CheckpointsMode == Checkpoints::STRICT && !cpSatisfies)
            return state.DoS(100, error("AcceptBlock() : rejected by synchronized checkpoint"),
                             REJECT_CHECKPOINT, "rejected by synchronized checkpoint");
        ...
        if (pcheckpoint && nHeight < pcheckpoint->nHeight)
            return state.DoS(100, error("AcceptBlock() : forked chain older than last checkpoint (height %d)", nHeight),
                             REJECT_CHECKPOINT, "forked chain older than last checkpoint");
    ...
    if (pcheckpoint && pblock->hashPrevBlock != (chainActive.Tip() ? chainActive.Tip()->GetBlockHash() : uint256(0)) && !Checkpoints::WantedByPendingSyncCheckpoint(hash))
    {
        ...
        if (pblock->GetBlockTime() > CHECK_POW_FROM_NTIME && bnNewBlock > bnRequired)
        {
            if (!pblock->IsProofOfStake())
                ...
                return state.DoS(100, error("ProcessBlock() : block with too little proof-of-stake"),
                                 REJECT_INVALID, "invalid pos");
        }
    }
