...
public:
    ...
    inline static size_t SerializationOp(T thisPtr, Stream& s, Operation ser_action, int nType, int nVersion) {
        ...
        if (thisPtr->nVersion > POW_TX_VERSION)
        {
            // PoSV
            READWRITE(*const_cast<uint32_t*>(&thisPtr->nTime));
        }
        else if (fRead)
        {
        	*const_cast<uint32_t*>(&thisPtr->nTime) = 0;
        }
        if (fRead)
            thisPtr->UpdateHash();
...
public:
    ...
    inline static size_t SerializationOp(T thisPtr, Stream& s, Operation ser_action, int nType, int nVersion) {
        ...
        // PoSV
        if (thisPtr->nVersion > POW_BLOCK_VERSION)
            READWRITE(thisPtr->vchBlockSig);
