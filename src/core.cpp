...
{
    std::string result = FormatMoney(nSatoshisPerK) + " RDD/kB";
    return result;
}
...
{
    LogPrintf("CBlock(hash=%s, PoW=%s, ver=%d, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, nBits=%08x, nNonce=%u, vtx=%u, vchBlockSig=%s)\n",
        GetHash().ToString(),
