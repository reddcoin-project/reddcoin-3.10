...
CAmount AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 46000000000.0)
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Reddcoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Reddcoin server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "mining",             "getstakinginfo",         &getstakinginfo,         true,      false,      false },
    ...
    /* Wallet updates */
    { "updates",            "checkforupdates",        &checkforupdates,        true,      false,      false },
    ...
    { "wallet",             "getinterest",            &getinterest,            false,     false,      true },
    ...
    { "wallet",             "reservebalance",         &reservebalance,         false,     false,      true },
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use reddcoind, or the -server option to reddcoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:45443/\n";
}
