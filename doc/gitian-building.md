...
Gitian is the deterministic build process that is used to build the Reddcoin
...
Clone the git repositories for reddcoin and gitian,
...
To build Reddcoin (for Linux and/or Windows) just follow the steps under 'perform
...
./bin/gbuild --commit bitcoin=${COMMIT} --url reddcoin=${URL} ../reddcoin/contrib/gitian-descriptors/gitian-win.yml
```
...
[reddcoin/gitian.sigs](https://github.com/reddcoin-project/gitian.sigs/) repository, or if not possible create a pull
request. You can also mail the files to me () and I'll commit them.
