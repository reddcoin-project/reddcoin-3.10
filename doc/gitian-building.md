...
Follow the instructions in [doc/release-process.md](release-process.md) in the reddcoin repository
...
To build Reddcoin (for Linux, OS X and Windows) just follow the steps under 'perform
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/reddcoin-build.assert
